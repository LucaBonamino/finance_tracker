...
# Specify the origins that are allowed to communicate with your API.
origins = [
    "http://localhost:1234",  # Replace with your frontend's URL
    "http://127.0.0.1:1234",  # You might have this as well
...
app.add_middleware(
    ...
    allow_credentials=True,  # If you need to support credentials (cookies, headers, etc.)
            ...
            try:
                for item in transactions.root:
                    ...
                    if item.category is not None:
                        cat = session.query(TransactionCategory).filter(TransactionCategory.category == item.category).one_or_none()
                        if cat is None:
